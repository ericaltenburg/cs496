...
and
  ...
  | Proj(e,id) ->
    ...
    | Some ev -> 	if (fst ev)
    				then int_of_refVal (snd ev) >>= fun r ->
    					(match (Store.deref g_store r) with
    					| None -> error "index out of bounds"
    					| Some some_val -> return some_val) (* If you're reading this, lots of frustration ensued because some_val was originally val which is reserved *)
    				else return (snd ev))
  | SetField(e1,id,e2) ->
  	eval_expr e1 >>= 
  	fields_of_recordVal >>= fun fs ->
  	(match (List.assoc_opt id fs) with
  		| None -> error (id^" not found")
  		| Some ev -> 	if (fst ev)
  						then int_of_refVal (snd ev) >>= fun r -> eval_expr e2 >>= fun e ->
  							(match (Store.set_ref g_store r e) with
  							| None -> error "index out of bounds"
  							| Some _ -> return UnitVal)
