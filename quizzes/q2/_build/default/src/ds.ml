...
let sequence_helper : (exp_val ea_result) list -> env -> (exp_val list) result = fun v env ->
	let x = (List.map (fun item -> item env) v) in (
		let err = (List.find_opt (fun item -> match item with 
			| Ok _ -> false
			| Error _ -> true) x) in (
			match err with
			| None -> Ok (List.map (fun (Ok v) -> v) x)
			| Some (Error e) -> Error e))

let sequence : (exp_val ea_result) list -> (exp_val list) ea_result = fun rs ->
	match rs with
	| [] -> return []
	| _ -> sequence_helper rs 
