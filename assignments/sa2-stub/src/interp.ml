...
let rec lookup_method : string -> string -> class_env ->
  ...
  | (a,b)::tl -> 	if a = c_name
  					then 	(match b with
  							| (_,_,z) -> (match (List.assoc_opt m_name z) with
  											| None -> None
  											| Some m -> Some m))
  					else lookup_method c_name m_name tl
...
(*     sequence (List.map eval_expr es) >>= fun arguments ->
  ...
  | Send(e,m_name,es) ->
    ...
    | Some m -> apply_method m_name temp_self arguments m)
  | Self ->
    eval_expr (Var "_self")
  | Super(m_name,es) ->
    sequence (List.map eval_expr es) >>= fun arguments ->
    ...
    eval_expr (Var "_self") >>= fun slf ->
    (match (lookup_method c_name m_name !g_class_env) with
    | None -> error "Method not found"
    | Some m -> apply_method m_name slf arguments m)
